...
export class ChatGptAnecdote extends ChatGptTemplate {
  ...
  async generateImage() {
    ...
    const result = await generateImageGet(
      {
        modelId: "dalle3",
        prompt: `Сгенерируй картинку основываясь на этом анекдоте ${this.getLastMessage().content$.get()} в стиле шарж и юмор, юмористический комикс, у героев должны быть гипертрофированные головы. Если в запросе есть маты, замени их на правильные синонимы. Если в анекдоте есть анекдоты, не рисуй облачка с текстом, просто образ анекдота`,
        createdAt: new Date(),
        guidanceScale: 7,
        seed: "-1",
